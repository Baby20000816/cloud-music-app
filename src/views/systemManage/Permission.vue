            ...
            <template>
              ...
              <mu-list-item-action>
                <mu-button
                  color="error"
                  style="height:30px;"
                  @click="deleteSingle(item.menu_id)"
                >删除</mu-button>
              </mu-list-item-action>
              <mu-list-item
              ...
              >
                ...
                <mu-list-item-action>
                  <mu-button
                    color="error"
                    style="height:30px;"
                    @click="deleteSingle(item1.menu_id)"
                  >删除</mu-button>
                </mu-list-item-action>
                <!-- <mu-list-item-action>
              <mu-icon class="toggle-icon" size="24" value="keyboard_arrow_down" v-if="item1.subMenus.length > 1"></mu-icon>
                ...
                <mu-list-item
                  ...
                  style="border-top:1px solid black"
                >
                  <mu-list-item-title style="margin-left:-20px">{{item.menu_id}}-{{index1+1}}-{{index2+1}}</mu-list-item-title>
                  ...
                  <mu-list-item-action>
                    <mu-button
                      color="error"
                      style="height:30px;"
                      @click="deleteSingle(item2.menu_id)"
                    >删除</mu-button>
                  </mu-list-item-action>
                </mu-list-item>
              </mu-list-item>
    ...
    <div class="view-menu">
      ...
      <p>
        <mu-select
          label="父类权限"
          class="input-width"
          v-model="permission.permissionParent"
        >
          <mu-option
      ...
      <p>
        <mu-text-field
          v-model="permission.permissionIcon"
          class="input-width"
          placeholder="图标"
        ></mu-text-field><br />
        <mu-text-field
          v-model="permission.permissionPath"
          class="input-width"
          placeholder="path"
        ></mu-text-field><br />
      </p>
